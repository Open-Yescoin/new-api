...
控制台创建的 `BytedToken` 只保存在发起浏览器的 `sessionStorage`。服务端数据库仅保存 SHA-256 摘要、发起用户、状态和时间戳，本地有效期固定为 15 分钟；创建新会话会使该用户之前的待处理会话失效。结果查询必须同时匹配当前用户和摘要，成功后在一个数据库事务中完成 `LivenessFace GroupId` 绑定并消费会话。
